...
// --- CONTROLLO RUOLO: solo Amministratore può modificare educatore ---
...
if ($stmtClasse) {
    ...
    if ($stmtClasse->fetch()) {
        if ($userClasse !== 'Amministratore') {
            echo json_encode(['success' => false, 'message' => 'Accesso negato. Solo Amministratore può modificare educatori.']);
            $stmtClasse->close();
            $conn->close();
            exit;
        }
