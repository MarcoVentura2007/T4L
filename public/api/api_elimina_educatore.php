...
// --- CONTROLLO RUOLO: solo Amministratore può eliminare educatore ---
...
if ($stmtClasse) {
    ...
    if ($stmtClasse->fetch()) {
        if ($userClasse !== 'Amministratore') {
            echo json_encode(['success' => false, 'message' => 'Accesso negato. Solo Amministratore può eliminare educatori.']);
            $stmtClasse->close();
            $conn->close();
            exit;
        }
