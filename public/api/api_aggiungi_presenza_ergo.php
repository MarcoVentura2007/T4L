...
// --- CONTROLLO RUOLO: solo Amministratore può aggiungere presenze ergo ---
...
if ($stmtClasse) {
    ...
    if ($stmtClasse->fetch()) {
        if ($userClasse !== 'Amministratore' && $userClasse !== 'Contabile' && $userClasse !== 'Educatore') {
            echo json_encode(['success' => false, 'message' => 'Accesso negato. Solo Amministratore può aggiungere presenze ergo.']);
            $stmtClasse->close();
            $connAccount->close();
            exit;
        }
...
if ($conn->connect_error) {
    echo json_encode(['success' => false, 'message' => 'Connessione DB ergo fallita']);
    exit;
}
