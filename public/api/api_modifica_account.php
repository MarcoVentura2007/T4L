...
// --- CONTROLLO RUOLO: solo Amministratore può modificare account ---
...
if ($stmtClasse) {
    ...
    if ($stmtClasse->fetch()) {
        if ($userClasse !== 'Amministratore') {
            echo json_encode(['success' => false, 'message' => 'Accesso negato. Solo Amministratore può modificare account.']);
            $stmtClasse->close();
            $conn->close();
            exit;
        }
